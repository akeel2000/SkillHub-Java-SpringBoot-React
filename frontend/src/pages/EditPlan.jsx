...
const EditPlan = () => {
  ...
  // Fetch the plan details when component mounts or id/token changes
  useEffect(() => {
  ...
  const handleUpdate = async () => {
    ...
    if (res.ok) {
      navigate("/learning-plans"); // Redirect on success
    } else {
      alert("Update failed");
    }
  };
  ...
  return (
    // Main background with floating circles
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 relative overflow-hidden p-6">
      ...
      {/* Custom CSS for floating and fade-in animation */}
      <style jsx global>{`
        @keyframes float {
